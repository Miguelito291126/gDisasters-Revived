...
if (SERVER) then 
	...
	util.AddNetworkString("gdr_env_dynamicwater_b_errorcheck")

end 


net.Receive("gdr_env_dynamicwater_b_errorcheck", function()
	
	local CurrentDistanceTravelled = net.ReadFloat()
	local ent = ents.FindByClass("gdr_env_dynamicwater_b")[1] 
...
function ENT:ErrorCheck()
	...
	if CurTime() >= self.NextErrorCheck then
		...
		net.Start("gdr_env_dynamicwater_b_errorcheck")
			...
			if (vpos.x <= max_x and vpos.x >= min_x) and (vpos.y <= max_y and vpos.y >= min_y) and (vpos.z <= (zrange + minz) and vpos.z >= minz) then 
				...
				elseif v.IsInWater and v:IsNPC() or v:IsNextBot() then
					...
					if v.IsInWater then
						...
						if #ents.FindByClass("gdr_d2_minivolcano*") >= 1 then return end
						if #ents.FindByClass("gdr_d8_ancientvolcano*") >= 1 then return end
						if #ents.FindByClass("gdr_d4_youngvolcano*") >= 1 then return end
						if #ents.FindByClass("gdr_d5_maturevolcano*") >= 1 then return end
...
function ENT:Think()
	...
	if (SERVER) then
		local t = GetConVar( "gdisasters_revived_envdynamicwater_simquality" ):GetFloat()-- tick dependant function that allows for constant think loop regardless of server tickrate
...
function ENT:OnWedgeEntry(ent) 
	...
	if ent:IsPlayer() or ent:IsNPC() or ent:IsNextBot() then 
		...
		if  GetConVar( "gdisasters_revived_envdynamicwater_candamageconstraints" ):GetInt()>=1 then
...
function createTsunami(parent, data)
	...
	for k, v in pairs(ents.FindByClass( "gdr_env_dynamiclava_b", "gdr_env_dynamicwater_b")) do
		v:Remove();
	end
	
	local tsunami = ents.Create("gdr_env_dynamicwater_b");
	tsunami.Data  = data;
...
water_textures[1]    = Material("nature/gdr_env_dynamicwater/base_water_01")
water_textures[2]    = Material("nature/gdr_env_dynamicwater/base_water_02")
water_textures[3]    = Material("nature/gdr_env_dynamicwater/base_water_03")


local water_shader   = {}
water_shader[1]    = Material("nature/gdr_env_dynamicwater/water_expensive_02")
water_shader[2]    = Material("nature/gdr_env_dynamicwater/water_expensive_01")


function gdr_env_dynamicwater_b_DrawWater()

	local tsunami = ents.FindByClass("gdr_env_dynamicwater_b")[1];
	...
	local water_shaders =  water_shader[ math.Clamp(GetConVar( "gdisasters_revived_graphics_water_shader_quality" ):GetInt(), 1, 2)]
	...
	RenderFix()
	if GetConVar( "gdisasters_revived_graphics_water_quality" ):GetInt() > 3 then DrawHQWater() else DrawLQWater()	end 
	model:Remove()	
end
