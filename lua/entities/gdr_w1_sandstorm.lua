...
function ENT:Initialize()		
	...
	if (SERVER) then
		...
		gDisasters_Revived.GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(15,35),["Direction"]=Vector(math.random(-1,1),math.random(-1,1),0)}, ["Pressure"]    = 101000, ["Temperature"] = math.random(35,45), ["Humidity"]    = math.random(1,15), ["BRadiation"]  = 0.1, ["Oxygen"]  = 100}}
...
function ENT:Think()
	if (CLIENT) then
		...
		local muffled_volume = math.Clamp(1 - ( LocalPlayer().gDisasters_Revived.Fog.Data.DensityCurrent/0.8), 0, 1)
