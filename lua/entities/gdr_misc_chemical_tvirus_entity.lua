...
function ENT:Think()	
	...
	if (self.Seconds >= 130) then -- Zombie time hehe
		if not self:IsValid() then return end
		if (file.Exists( "lua/autorun/vj_nmrih_autorun.lua", "GAME" )) and GetConVar("gdisasters_revived_tvirus_nmrih_zombies"):GetInt()== 1 then
			local ent = ents.Create(table.Random(ZombieList2)) -- This creates our zombie entity
			ent:SetPos(self.infected:GetPos())
			ent:Spawn() 
			if GetConVar("gdisasters_revived_tvirus_zombie_strength"):GetInt() == 0 then
				ent:SetHealth(500)
			elseif GetConVar("gdisasters_revived_tvirus_zombie_strength"):GetInt() == 1 then
				ent:SetHealth(1000)
			elseif GetConVar("gdisasters_revived_tvirus_zombie_strength"):GetInt() == 2 then
				ent:SetHealth(2000)
			elseif GetConVar("gdisasters_revived_tvirus_zombie_strength"):GetInt() == -1 then
				ent:SetHealth(250)
			elseif GetConVar("gdisasters_revived_tvirus_zombie_strength"):GetInt() == -2 then
