...
local chaseMusic = Sound("streams/nextbot/npc_xshitter/xshitter.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_xshitter_acquire_distance =
	CreateConVar("npc_xshitter_acquire_distance", 9500, FCVAR_NONE,
	"The maximum distance at which xshitter will chase a target.")

local npc_xshitter_spawn_protect =
	CreateConVar("npc_xshitter_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, xshitter will not target players or hide within 200 units of \z
	a spawn point.")

local npc_xshitter_attack_distance =
	CreateConVar("npc_xshitter_attack_distance", 60, FCVAR_NONE,
	"The reach of xshitter's attack.")

local npc_xshitter_attack_interval =
	CreateConVar("npc_xshitter_attack_interval", 0.1, FCVAR_NONE,
	"The delay between xshitter's attacks.")

local npc_xshitter_attack_force =
	CreateConVar("npc_xshitter_attack_force", 9000, FCVAR_NONE,
	"The physical force of xshitter's attack. Higher values throw things \z
	farther.")

local npc_xshitter_smash_props =
	CreateConVar("npc_xshitter_smash_props", 1, FCVAR_NONE,
	"If set to 1, xshitter will punch through any props placed in their way.")

local npc_xshitter_allow_jump =
	CreateConVar("npc_xshitter_allow_jump", 1, FCVAR_NONE,
	"If set to 1, xshitter will be able to jump.")

local npc_xshitter_hiding_scan_interval =
	CreateConVar("npc_xshitter_hiding_scan_interval", 3, FCVAR_NONE,
...
local npc_xshitter_hiding_repath_interval =
	CreateConVar("npc_xshitter_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to xshitter's hiding spot will be redetermined every X seconds.")

local npc_xshitter_chase_repath_interval =
	CreateConVar("npc_xshitter_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of xshitter's target will be redetermined every \z
	X seconds.")

local npc_xshitter_expensive_scan_interval =
	CreateConVar("npc_xshitter_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_xshitter_force_download =
	CreateConVar("npc_xshitter_force_download", 1, FCVAR_ARCHIVE,
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_xshitter: found %d suitable (%d unsuitable) hiding \z
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_xshitter: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_xshitter: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_xshitter: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_xshitter: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_xshitter: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, xshitter in pairs(ents.FindByClass("npc_xshitter")) do
		xshitter:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		print("npc_xshitter: nav_generate failed to initialize")
...
concommand.Add("npc_xshitter_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_xshitter: Beginning nav_generate requested by " .. ply:Name())
	...
	elseif isConsole then
		print("npc_xshitter: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_xshitter_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_xshitter_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_xshitter_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_xshitter_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_xshitter_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_xshitter_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_xshitter_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_xshitter_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_xshitter_attack_distance:GetInt()
		...
		local repathInterval = npc_xshitter_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_xshitter_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_xshitter_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_xshitter_hiding_repath_interval:GetFloat()
...
local MAT_xshitter = Material("nextbot/npc_xshitter/xshitter")
killicon.Add("npc_xshitter", "nextbot/npc_xshitter/xshitter", color_white)
...
local npc_xshitter_music_volume =
	CreateConVar("npc_xshitter_music_volume", 1,
...
local function updatePanicMusic()
	if #ents.FindByClass("npc_xshitter") == 0 then
	...
	local userVolume = math.Clamp(npc_xshitter_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_xshitter" then
...
local function requestNavGenerate()
	RunConsoleCommand("npc_xshitter_learn")
end
...
net.Receive("xshitter_nag", function()
	...
	if game.SinglePlayer() then
		...
		Derma_Query("Uh oh! xshitter doesn't know this map. \z
			...
			\n\z
			If you ARE the server host, you can run npc_xshitter_learn over \z
			rcon.\n\z
