...
local chaseMusic = Sound("streams/nextbot/gdr_dnpc_rainbow/rainbow.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local gdr_dnpc_rainbow_acquire_distance =
	CreateConVar("gdr_dnpc_rainbow_acquire_distance", 9500, FCVAR_NONE,
	"The maximum distance at which rainbow will chase a target.")

local gdr_dnpc_rainbow_spawn_protect =
	CreateConVar("gdr_dnpc_rainbow_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, rainbow will not target players or hide within 200 units of \z
	a spawn point.")

local gdr_dnpc_rainbow_attack_distance =
	CreateConVar("gdr_dnpc_rainbow_attack_distance", 60, FCVAR_NONE,
	"The reach of rainbow's attack.")

local gdr_dnpc_rainbow_attack_interval =
	CreateConVar("gdr_dnpc_rainbow_attack_interval", 0.1, FCVAR_NONE,
	"The delay between rainbow's attacks.")

local gdr_dnpc_rainbow_attack_force =
	CreateConVar("gdr_dnpc_rainbow_attack_force", 9000, FCVAR_NONE,
	"The physical force of rainbow's attack. Higher values throw things \z
	farther.")

local gdr_dnpc_rainbow_smash_props =
	CreateConVar("gdr_dnpc_rainbow_smash_props", 1, FCVAR_NONE,
	"If set to 1, rainbow will punch through any props placed in their way.")

local gdr_dnpc_rainbow_allow_jump =
	CreateConVar("gdr_dnpc_rainbow_allow_jump", 1, FCVAR_NONE,
	"If set to 1, rainbow will be able to jump.")

local gdr_dnpc_rainbow_hiding_scan_interval =
	CreateConVar("gdr_dnpc_rainbow_hiding_scan_interval", 3, FCVAR_NONE,
...
local gdr_dnpc_rainbow_hiding_repath_interval =
	CreateConVar("gdr_dnpc_rainbow_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to rainbow's hiding spot will be redetermined every X seconds.")

local gdr_dnpc_rainbow_chase_repath_interval =
	CreateConVar("gdr_dnpc_rainbow_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of rainbow's target will be redetermined every \z
	X seconds.")

local gdr_dnpc_rainbow_expensive_scan_interval =
	CreateConVar("gdr_dnpc_rainbow_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local gdr_dnpc_rainbow_force_download =
	CreateConVar("gdr_dnpc_rainbow_force_download", 1, FCVAR_ARCHIVE,
...
local function buildHidingSpotCache()
	...
	print(string.format("gdr_dnpc_rainbow: found %d suitable (%d unsuitable) hiding \z
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("gdr_dnpc_rainbow: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("gdr_dnpc_rainbow: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("gdr_dnpc_rainbow: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("gdr_dnpc_rainbow: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("gdr_dnpc_rainbow: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, rainbow in pairs(ents.FindByClass("gdr_dnpc_rainbow")) do
		rainbow:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		print("gdr_dnpc_rainbow: nav_generate failed to initialize")
...
concommand.Add("gdr_dnpc_rainbow_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("gdr_dnpc_rainbow: Beginning nav_generate requested by " .. ply:Name())
	...
	elseif isConsole then
		print("gdr_dnpc_rainbow: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = gdr_dnpc_rainbow_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `gdr_dnpc_rainbow_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if gdr_dnpc_rainbow_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = gdr_dnpc_rainbow_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not gdr_dnpc_rainbow_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = gdr_dnpc_rainbow_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = gdr_dnpc_rainbow_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = gdr_dnpc_rainbow_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = gdr_dnpc_rainbow_attack_distance:GetInt()
		...
		local repathInterval = gdr_dnpc_rainbow_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and gdr_dnpc_rainbow_allow_jump:GetBool()
		...
		local hidingScanInterval = gdr_dnpc_rainbow_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = gdr_dnpc_rainbow_hiding_repath_interval:GetFloat()
...
local MAT_rainbow = Material("nextbot/gdr_dnpc_rainbow/rainbow")
killicon.Add("gdr_dnpc_rainbow", "nextbot/gdr_dnpc_rainbow/rainbow", color_white)
language.Add("gdr_dnpc_rainbow", "rainbow ")
...
local gdr_dnpc_rainbow_music_volume =
	CreateConVar("gdr_dnpc_rainbow_music_volume", 1,
...
local function updatePanicMusic()
	if #ents.FindByClass("gdr_dnpc_rainbow") == 0 then
	...
	local userVolume = math.Clamp(gdr_dnpc_rainbow_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "gdr_dnpc_rainbow" then
...
local function requestNavGenerate()
	RunConsoleCommand("gdr_dnpc_rainbow_learn")
end
...
net.Receive("rainbow_nag", function()
	...
	if game.SinglePlayer() then
		...
		Derma_Query("Uh oh! rainbow doesn't know this map. \z
			...
			\n\z
			If you ARE the server host, you can run gdr_dnpc_rainbow_learn over \z
			rcon.\n\z
...
list.Set( "NPC", "gdr_dnpc_rainbow", {
	Name = "rainbowdoesn'tstuff", 
	Class = "gdr_dnpc_rainbow", 
	Category = "Nextbot",
	AdminOnly = false,
})
