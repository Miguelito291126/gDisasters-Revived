...
function ENT:SpawnStar()
	...
	if gDisasters_Revived:HitChance(24) then
		...
		if #ents.FindByClass("gdr_d2_shootingstar") < 1 then 
		
		
			

			local star = ents.Create("gdr_d2_shootingstar_ch")
			star:Spawn()
			star:Activate()	
...
function ENT:SpawnFunction( ply, tr )
	...
	if #ents.FindByClass("gdr_d2_shootingstarshower") >= 1 then return end
