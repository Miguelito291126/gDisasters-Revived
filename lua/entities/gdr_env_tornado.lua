...
function ENT:SetGroundSpeed()
	...
	if GetConVar( "gdisasters_revived_envtornado_manualspeed" ):GetInt() >= 1 then 
	
		self.GroundSpeed = GetConVar( "gdisasters_revived_envtornado_speed" ):GetInt()
...
function ENT:Waterspout(dir)
	...
	local entity = ents.FindByClass("gdr_d2_waterspout", "gdr_d2_waterspout_pathed")[1]
...
function ENT:EvolvingTornado()
	local entity = ents.FindByClass("gdr_d6_evolvingtornado")[1]
...
function ENT:Landspout(dir)
	...
	local entity = ents.FindByClass("gdr_d4_landspout", "gdr_d4_landspout_pathed")[1]
...
function ENT:Snownado(dir)
	...
	local entity = ents.FindByClass("gdr_d3_snownado", "gdr_d3_snownado_pathed")[1]
...
function ENT:TryRemoveConstraints(ent)
	...
	if GetConVar( "gdisasters_revived_envtornado_candamageconstraints" ):GetInt()!=1 then return end
	...
	if self.Data.EnhancedFujitaScale == "EF0" then
		chance = GetConVar( "gdisasters_revived_envtornado_damage" ):GetInt()
...
function ENT:Physics()
	local phys_scalar = GetConVar( "gdisasters_revived_envtornado_simquality" ):GetFloat() / 0.01
	...
	self.NextPhysicsTime = CurTime() + GetConVar( "gdisasters_revived_envtornado_simquality" ):GetFloat()
end
...
function createTornado(data)
	...
	local tornado = ents.Create("gdr_env_tornado")
	tornado.Data  = data
	tornado:Spawn()
	tornado:Activate()
...
function ENT:Decay()
	timer.Simple(math.random(GetConVar("gdisasters_revived_envtornado_lifetime_min"):GetInt(), GetConVar("gdisasters_revived_envtornado_lifetime_max"):GetInt()), function()
		if !self:IsValid() then return end
		self:Remove()
	end)
end
