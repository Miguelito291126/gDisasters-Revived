...
function ENT:VolcanicLavaEffects()
	...
	if gDisasters_Revived:HitChance(10) then ParticleEffect(effect, pos, Angle(0,0,0), nil) end
...
function ENT:PressureIncrement()
	...
	if self.IsGoingToErupt==false and self.IsPressureLeaking == false then
		self.Pressure = math.Clamp(self.Pressure + GetConVar("gdisasters_revived_volcano_pressure_increase"):GetFloat() ,0,100)
		
	elseif self.IsGoingToErupt == false and self.IsPressureLeaking == true then
		self.Pressure = math.Clamp(self.Pressure - GetConVar("gdisasters_revived_volcano_pressure_decrease"):GetFloat() ,0,100)
	...
	if self.Pressure >= 100 then
		if self.IsGoingToErupt==false then
			...
			if math.random(1,1)==1 then
				...
				local earthquake = ents.Create(table.Random({"gdr_d9_rs9eq","gdr_d10_rs10eq"}))
...
function ENT:CreateRocks(num, lifetime)
	...
	for i=0, num do
		...
		local rock = ents.Create("gdr_d6_volcanic_rock_ch")
		rock:SetPos( pos ) 
		rock:Spawn()
		rock:Activate()
...
function ENT:Erupt()
	timer.Simple(2, function() -- we have a delay here because air is still expanding from heat
		...
		gDisasters_Revived:CreateSoundWave("streams/disasters/nature/krakatoa_explosion.mp3", self:GetPos(), "3d" ,340.29/2, {100,100}, 5)
	end)
	...
	timer.Simple(140, function()
		local ent2 = ents.FindByClass("gdr_dw2_ashstorm")[1]
		local ent3 = ents.FindByClass("gdr_d7_lavabombshower")[1]
		local ent4 = ents.FindByClass("gdr_d8_volcanicrockhower")[1]
		...
		local ent = ents.Create("gdr_dw4_heavyacidrain")
		ent:Spawn()
		ent:Activate()
		
	end)
	timer.Simple(300, function()
		local ent = ents.FindByClass("gdr_dw4_heavyacidrain")[1]
