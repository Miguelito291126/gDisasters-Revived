...
if (SERVER) then 
	...
	util.AddNetworkString("gdr_env_dynamiclava_b_errorcheck")

end 


net.Receive("gdr_env_dynamiclava_b_errorcheck", function()
	
	local CurrentDistanceTravelled = net.ReadFloat()
	local ent = ents.FindByClass("gdr_env_dynamiclava_b")[1] 
...
function ENT:ErrorChecklava()
	...
	if CurTime() >= self.NextErrorCheck then
		...
		net.Start("gdr_env_dynamiclava_b_errorcheck")
...
function createTsunamilava(parent, data)
	if gDisasters_Revived.IsMapRegistered() == true then
		...
		local tsunamilava = ents.Create("gdr_env_dynamiclava_b");
...
local lava_textures = {}
lava_textures[1] = Material("nature/gdr_env_dynamiclava/base_lava_01")
lava_textures[2] = Material("nature/gdr_env_dynamiclava/base_lava_02")

function gdr_env_dynamiclava_b_DrawLava()

	local tsunamilava = ents.FindByClass("gdr_env_dynamiclava_b")[1];
