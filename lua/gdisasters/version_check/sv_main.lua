...
local function RunCheck()
    http.Fetch(gDisasters_Revived.WorkShopURL, function(code)
        ...
        cookie.Set("gdr_dnextv", lV)
    end)
end
local function RunLogic()
    -- Check if a newer version is out
    local lV = cookie.GetNumber("gdr_dnextv", gDisasters_Revived.Version)
    if cookie.GetNumber("gdr_dnextvcheck", 0) > os.time() then
        if lV > gDisasters_Revived.Version then
            gDisasters_Revived:Msg("Version " .. lV .. " is out!")
        end
    else
        RunCheck()
        cookie.Set("gdr_dnextvcheck", os.time() + 129600) -- Check in 1½ day
    end
end
