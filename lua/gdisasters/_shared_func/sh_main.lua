...
function gDisasters_Revived:gDisasters_Is3DSkybox()
	...
	if (SERVER) then 
		...
		gDisasters_Revived.Cached.SkyCamera = ents.FindByClass("sky_camera")

		if #gDisasters_Revived.Cached.SkyCamera > 0 then 
			return true 
		else 
			return IsValid(gDisasters_Revived.Cached.SkyCamera[1]) 
		end 
