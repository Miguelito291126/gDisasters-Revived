local function Autospawn_Timer()
	...
	local function Autospawn()
		...
		if GetConVar("gdisasters_revived_autospawn_type"):GetString() == "Tornado" then
			recent = true
			local tornado = {
				"gdr_d2_gustnado",
				"gdr_d3_ef0", 
				...
				"gdr_d4_icenado",  
				"gdr_d5_ef2", 
				"gdr_d5_alien_tornado",
				"gdr_d6_ef3", 
				"gdr_d6_evolvingtornado",
				"gdr_d7_ef4", 
				"gdr_d7_ef4_small", 
				"gdr_d8_ef5", 
				"gdr_d8_ef5_small", 
			...
			for k, ply in pairs(player.GetAll()) do
				if GetConVar("gdisasters_revived_autospawn_chat"):GetInt() <= 0 then return end
				ply:PrintMessage(HUD_PRINTTALK,"the disaster that is happening now: " .. EF.PrintName )
			end

		elseif GetConVar("gdisasters_revived_autospawn_type"):GetString() == "Disasters" then
			recent = true
			local disasters = {}
			...
			for k, v in pairs(ents.FindByClass("gdr_d*_ef*")) do

				dis:SetPos(SkyPos)

			end
			for k, v in pairs(ents.FindByClass("gdr_d*_*spout")) do
			...
			for k, v in pairs(ents.FindByClass("gdr_d*_*tsunami")) do

				dis:SetPos(FloodPos)

			end
			for k, v in pairs(ents.FindByClass("gdr_d*_tsunami")) do

				dis:SetPos(FloodPos)

			end
			for k, v in pairs(ents.FindByClass("gdr_d*_*flood")) do

				dis:SetPos(FloodPos)

			end
			for k, v in pairs(ents.FindByClass("gdr_d*_flood")) do

				dis:SetPos(FloodPos)

			end
			for k, v in pairs(ents.FindByClass("gdr_d*_*hole")) do

				dis:SetPos(CenterPos)

			end
			for k, v in pairs(ents.FindByClass("gdr_d*_neutron_star")) do
			...
			timer.Simple(GetConVar( "gdisasters_revived_autospawn_remove_timer" ):GetInt(), function()
				if dis:IsValid() then dis:Remove() end
			end)
		
		elseif GetConVar("gdisasters_revived_autospawn_type"):GetString() == "Weather" then
			recent = true
			local weather = {}

			local function TableAdd()
				local files, directory = file.Find("entities/gdr_dw*.lua", "LUA")
		...
		elseif GetConVar("gdisasters_revived_autospawn_type"):GetString() == "Weather/Disasters" then
			recent = true
			local weadisas = {}

			local function TableAdd()
				local files, directory = file.Find("entities/gdr_d*.lua", "LUA")
				local files2, directory2 = file.Find("entities/gdr_dw*.lua", "LUA")
			...
			for k, v in pairs(ents.FindByClass("gdr_dw*")) do
