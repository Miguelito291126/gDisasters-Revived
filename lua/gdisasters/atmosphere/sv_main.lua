SetGlobalFloat("gDisasters_Revived_Temperature", 0)
SetGlobalFloat("gDisasters_Revived_Pressure", 0)
SetGlobalFloat("gDisasters_Revived_Humidity", 0)
SetGlobalFloat("gDisasters_Revived_BRadiation", 0)
SetGlobalFloat("gDisasters_Revived_Oxygen", 0)
SetGlobalFloat("gDisasters_Revived_wind", 0)
SetGlobalVector("gDisasters_Revived_wind_Direction", Vector(1,0,0))
...
function Temperature()
	...
	SetGlobalFloat("gDisasters_Revived_Temperature", temp)

	if GetConVar("gdisasters_revived_hud_temp_enable"):GetInt() <= 0 then return end
	...
	local function updateVars()
		...
		for k, v in pairs(plytbl) do
			...
			local core_equilibrium           =  math.Clamp((37 - v.gDisasters_Revived.Body.Temperature)*body_heat_genK, -body_heat_genMAX, body_heat_genMAX)
			...
			local ambient_equilibrium        = math.Clamp(((temp - v.gDisasters_Revived.Body.Temperature)*body_heat_genK), -body_heat_genMAX*1.1, body_heat_genMAX * 1.1)
		...
		for k, v in pairs(plytbl) do
			...
			if math.random(1,25) == 25 then
				if alpha_cold != 0 then
					...
					if GetConVar("gdisasters_revived_hud_temp_player_speed_enable"):GetInt() == 0 then return end
				...
				elseif alpha_hot != 0 then
					...
					v:SetWalkSpeed(GetConVar("gdisasters_revived_hud_temp_player_speed_walk"):GetInt())
					v:SetRunSpeed(GetConVar("gdisasters_revived_hud_temp_player_speed_sprint"):GetInt())
				end
			end
			
			if temp <= -100 then
				if outdoor then
					v.gDisasters_Revived.Body.Temperature = v.gDisasters_Revived.Body.Temperature - 0.01
				else
					v.gDisasters_Revived.Body.Temperature = v.gDisasters_Revived.Body.Temperature - 0.001	
				end
			...
			if temp <= -500 then
				if outdoor then
					v.gDisasters_Revived.Body.Temperature = v.gDisasters_Revived.Body.Temperature - 0.1
				else
					v.gDisasters_Revived.Body.Temperature = v.gDisasters_Revived.Body.Temperature - 0.01	
				end
			...
			if temp <= -1000 then
				if outdoor then
					v.gDisasters_Revived.Body.Temperature = v.gDisasters_Revived.Body.Temperature - 1
				else
					v.gDisasters_Revived.Body.Temperature = v.gDisasters_Revived.Body.Temperature - 0.1	
				end
			...
			if temp >= -273.3 and temp <= 4 then
				if wl==false then
				elseif lv==true then
					v.gDisasters_Revived.Body.Temperature = v.gDisasters_Revived.Body.Temperature + 0.004
				else
					v.gDisasters_Revived.Body.Temperature = v.gDisasters_Revived.Body.Temperature - 0.004
				end
	...
	local function subzero_Effect()
		if GetConVar("gdisasters_revived_hud_temp_breathing"):GetInt() == 0 then return end
	...
	local function GlobalBreathingEffect()
		if temp > 0 or GetConVar("gdisasters_revived_hud_temp_breathing"):GetInt() == 0 then 
		...
		for k, v in pairs(player.GetAll()) do
			...
			if gDisasters_Revived:isUnderWater(v) or gDisasters_Revived:isUnderLava(v) then 
				...
				if v.gDisasters_Revived.Body.Oxygen <= 0 then
					...
					if GetConVar("gdisasters_revived_hud_oxygen_damage"):GetInt() == 0 then return end
...
function Wind()
	...
	SetGlobalFloat("gDisasters_Revived_wind", windspeed)
	SetGlobalVector("gDisasters_Revived_wind_Direction", winddir)
	...
	if GetConVar("gdisasters_revived_stormfox_enabled"):GetInt() >= 1 then 
		...
		if Stormfox then 
			...
			if StormFox.IsThunder() then
				local ent = ents.FindByClass("gdr_d3_lightningstorm")[1]
...
function spacebuild()
	...
	if CAF or LS then  
		for k, v in pairs(player.GetAll()) do
			v.gDisasters_Revived.Body.Oxygen = v.suit.air
