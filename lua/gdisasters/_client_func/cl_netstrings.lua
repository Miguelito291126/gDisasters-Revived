...
net.Receive("gdr_screen_particles", function()
	...
	if GetConVar("gdisasters_revived_graphics_enable_manual_number_of_screen_particles"):GetInt() == 1 then
		number   = math.random(0,GetConVar("gdisasters_revived_graphics_number_of_screen_particles"):GetFloat())
	else
...
net.Receive("gdr_soundwave", function()
	...
	elseif stype == "stereo" then
		LocalPlayer():EmitSound( s, 100, math.random(pitchrange[1], pitchrange[2]), GetConVar("gdisasters_revived_volume_soundwave"):GetFloat() )
	elseif stype == "3d" then
		sound.Play( s,  pos, 150, math.random(pitchrange[1], pitchrange[2]), GetConVar("gdisasters_revived_volume_soundwave"):GetFloat() )
	end
...
net.Receive("gdr_isOutdoor", function()
	...
	if isOutside then
		...
		LocalPlayer().gDisasters_Revived.Outside.OutsideFactor   = Lerp( 0.01, LocalPlayer().gDisasters_Revived.Outside.OutsideFactor, 100)

		LocalPlayer().gDisasters_Revived.Fog.Data.DensityCurrent =  Lerp( 0.02, LocalPlayer().gDisasters_Revived.Fog.Data.DensityCurrent, LocalPlayer().gDisasters_Revived.Fog.Data.DensityMax )
		LocalPlayer().gDisasters_Revived.Fog.Data.EndMinCurrent  =  Lerp( 0.02, LocalPlayer().gDisasters_Revived.Fog.Data.EndMinCurrent, LocalPlayer().gDisasters_Revived.Fog.Data.EndMin )
	else
		LocalPlayer().gDisasters_Revived.Outside.OutsideFactor   = Lerp( 0.01, LocalPlayer().gDisasters_Revived.Outside.OutsideFactor, 0)
		LocalPlayer().gDisasters_Revived.Fog.Data.DensityCurrent =  Lerp( 0.01, LocalPlayer().gDisasters_Revived.Fog.Data.DensityCurrent, LocalPlayer().gDisasters_Revived.Fog.Data.DensityMin )
		LocalPlayer().gDisasters_Revived.Fog.Data.EndMinCurrent  =  Lerp( 0.01, LocalPlayer().gDisasters_Revived.Fog.Data.EndMinCurrent, LocalPlayer().gDisasters_Revived.Fog.Data.EndMax )
	end
...
net.Receive("gdr_createfog", function()
	...
	LocalPlayer().gDisasters_Revived.Fog.Data   = info
...
net.Receive( "gdr_entity_exists_on_server", function() 
	...
	if !IsValid(gDisasters_Revived.CachedExists[string]) then
		gDisasters_Revived.CachedExists[string] = ents.Create(string)
		gDisasters_Revived.CachedExists[string]:Spawn()
		gDisasters_Revived.CachedExists[string]:Activate()
	end
end)
