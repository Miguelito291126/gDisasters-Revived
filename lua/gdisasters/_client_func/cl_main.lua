...
hook.Add("RenderScreenspaceEffects", "gfx_Underlava", function() 
	...
	if gDisasters_Revived.isUnderLava(LocalPlayer()) then	
		...
		local flood2  = ents.FindByClass("gdr_env_dynamiclava")[1] or ents.FindByClass("gdr_env_dynamiclava_b")[1]
	...
	timer.Simple(t, function()
		if LocalPlayer():IsValid() then
			...
			elseif soundtype == "stereo" then
				LocalPlayer():EmitSound( soundpath, 100, math.random(pitchrange[1], pitchrange[2]), GetConVar("gdisasters_revived_volume_soundwave"):GetFloat() )
			elseif soundtype == "3d" then
				sound.Play( soundpath,  epicenter, 170, math.random(pitchrange[1], pitchrange[2]), GetConVar("gdisasters_revived_volume_soundwave"):GetFloat() )
			end		
		end
	end)
