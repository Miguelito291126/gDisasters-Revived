...
function hud_gDisastersINFO()
	if GetConVar( "gdisasters_revived_hud_enabled" ):GetInt()!=1 then return end
	
	if GetConVar( "gdisasters_revived_hud_type" ):GetInt()==1 then

		hud_DrawBasicINFO()	

	elseif GetConVar( "gdisasters_revived_hud_type" ):GetInt()==2 then
	
		hud_DrawBarometer()

	elseif GetConVar( "gdisasters_revived_hud_type" ):GetInt()==3 then
	
		hud_DrawSeismoGraph()
	elseif GetConVar( "gdisasters_revived_hud_type" ):GetInt()==4 and GetConVar("gdisasters_revived_spacebuild_enabled"):GetInt() >= 1 then
		
		hud_DrawSpacebuildINFO()
	elseif GetConVar( "gdisasters_revived_hud_type" ):GetInt()==5 and GetConVar( "gdisasters_revived_heat_system_enabled" ):GetInt()==1 then
...
function hud_gDisastersAlarm()
	...
	if CurTime() >= LocalPlayer().gDisasters_Revived.HUD.NextWarningSoundTime then
	
		LocalPlayer():EmitSound( "streams/disasters/player/hud_warning.wav", 100, 100, math.Clamp(GetConVar( "gdisasters_revived_volume_hud_warning" ):GetFloat(),0,1) )
		LocalPlayer().gDisasters_Revived.HUD.NextWarningSoundTime = CurTime() + 1
	else
	end
end

function hud_gDisastersHeart(nexttime, pitch)
	if CurTime() >= LocalPlayer().gDisasters_Revived.HUD.NextHeartSoundTime then
		LocalPlayer():EmitSound( "streams/disasters/player/heartbeat.wav", 100, pitch, math.Clamp(GetConVar( "gdisasters_revived_volume_hud_heartbeat" ):GetFloat(),0,1))
		LocalPlayer().gDisasters_Revived.HUD.NextHeartSoundTime = CurTime() + nexttime
	else
	end
end
...
function hud_gDisastersVomit()
	...
	if CurTime() >= LocalPlayer().gDisasters_Revived.HUD.NextVomitTime then
		LocalPlayer().gDisasters_Revived.HUD.VomitIntensity = 1
		net.Start("gdr_vomit")
		net.WriteEntity(LocalPlayer())
		net.SendToServer()
		
		LocalPlayer().gDisasters_Revived.HUD.NextVomitTime = CurTime() + math.random(1,19)
	else
	end
end
...
function hud_gDisastersVomitBlood()
	...
	if CurTime() >= LocalPlayer().gDisasters_Revived.HUD.NextVomitBloodTime  then
		LocalPlayer().gDisasters_Revived.HUD.BloodVomitIntensity   = 1
		net.Start("gdr_vomit_blood")
		net.WriteEntity(LocalPlayer())
		net.SendToServer()
		
		LocalPlayer().gDisasters_Revived.HUD.NextVomitBloodTime  = CurTime() + math.random(1,19)
	else
	end
end	
...
function hud_gDisastersSneeze()
	...
	if CurTime() >= LocalPlayer().gDisasters_Revived.HUD.NextSneezeTime  then
		LocalPlayer().gDisasters_Revived.HUD.SneezeIntensity   = 1
		net.Start("gdr_sneeze")
		net.WriteEntity(LocalPlayer())
		net.SendToServer()
		
		LocalPlayer().gDisasters_Revived.HUD.NextSneezeTime  = CurTime() + math.random(1,20)
	else
	end
end
...
function hud_gDisastersSneezeBig()
	...
	if CurTime() >= LocalPlayer().gDisasters_Revived.HUD.NextSneezeBigTime  then
		LocalPlayer().gDisasters_Revived.HUD.SneezeBigIntensity   = 1
		net.Start("gdr_sneeze_big")
		net.WriteEntity(LocalPlayer())
		net.SendToServer()
		
		LocalPlayer().gDisasters_Revived.HUD.NextSneezeBigTime  = CurTime() + math.random(1,24)
	else
	end
end
...
function hud_DrawBarometer()
    ...
    local function drawBarometerArrow()
        ...
        local pressure = GetGlobalFloat("gDisasters_Revived_Pressure")
...
function hud_DrawSpacebuildINFO()
	...
	local air_tmp   = math.Round(GetGlobalFloat("gDisasters_Revived_Temperature"),1)
	local body_oxygen = math.Round(LocalPlayer():GetNWFloat("gdr_BodyOxygen"))
	...
	local function drawText()
		...
		if GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°C" then
		...
		elseif GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°F" then
			draw.DrawText( "Air Temperature: ".. gDisasters_Revived.convert_CelciustoFahrenheit(air_temp).."°F", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature.x , pos_air_temperature.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
		elseif GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°K" then
			draw.DrawText( "Air Temperature: ".. gDisasters_Revived.convert_CelciustoKelvin(air_temp).."°K", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature.x , pos_air_temperature.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
		end
...
function hud_DrawHeatSystemINFO()
	...
	local function drawText()
		...
		if GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°C" then
			...
			draw.DrawText( language.GetPhrase("gdr_hud_wet_bulve_temperature")..Air_Temp_bh.."°C", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature_bh.x , pos_air_temperature_bh.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_dewpoint")..Dew_Point.."°C", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_dew_point.x , pos_dew_point.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_windchill")..Wind_Chill.."°C", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_wind_chill.x , pos_wind_chill.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_heatindex")..Heat_Index.."°C", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_Heat_Index.x , pos_Heat_Index.y, color, TEXT_ALIGN_LEFT )
		elseif GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°F" then
			
			draw.DrawText( language.GetPhrase("gdr_hud_wet_bulve_temperature")..gDisasters_Revived.convert_CelciustoFahrenheit(Air_Temp_bh) .."°F", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature_bh.x , pos_air_temperature_bh.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_dewpoint")..gDisasters_Revived.convert_CelciustoFahrenheit(Dew_Point).."°F", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_dew_point.x , pos_dew_point.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_windchill")..gDisasters_Revived.convert_CelciustoFahrenheit(Wind_Chill).."°F", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_wind_chill.x , pos_wind_chill.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_heatindex")..gDisasters_Revived.convert_CelciustoFahrenheit(Heat_Index).."°F", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_Heat_Index.x , pos_Heat_Index.y, color, TEXT_ALIGN_LEFT )
		
		elseif GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°K" then
			draw.DrawText( language.GetPhrase("gdr_hud_wet_bulve_temperature").. gDisasters_Revived.convert_CelciustoKelvin(Air_Temp_bh) .."°K", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature_bh.x , pos_air_temperature_bh.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_dewpoint").. gDisasters_Revived.convert_CelciustoKelvin(Dew_Point).."°K", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_dew_point.x , pos_dew_point.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_windchill").. gDisasters_Revived.convert_CelciustoKelvin(Wind_Chill).."°K", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_wind_chill.x , pos_wind_chill.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_heatindex").. gDisasters_Revived.convert_CelciustoKelvin(Heat_Index).."°K", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_Heat_Index.x , pos_Heat_Index.y, color, TEXT_ALIGN_LEFT )
		end

		if GetConVar("gdisasters_revived_hud_windtype"):GetString() == "km/h" then
			draw.DrawText( language.GetPhrase("gdr_hud_airflow")..Air_Flow.." km/h", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_Air_Flow.x , pos_Air_Flow.y, color, TEXT_ALIGN_LEFT )
		elseif GetConVar("gdisasters_revived_hud_windtype"):GetString() == "mph" then
			draw.DrawText( language.GetPhrase("gdr_hud_airflow")..gDisasters_Revived.convert_KMPHtoMPH(Air_Flow).." mph", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_Air_Flow.x , pos_Air_Flow.y, color, TEXT_ALIGN_LEFT )
		end
		
		
		draw.DrawText( language.GetPhrase("gdr_hud_latentheat")..Latent_Heat.." J/kg", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_latent_heat.x , pos_latent_heat.y, color, TEXT_ALIGN_LEFT )
		draw.DrawText( language.GetPhrase("gdr_hud_clouddensity")..cloud_Density.." g/m³", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_cloud_Density.x , pos_cloud_Density.y, color2, TEXT_ALIGN_LEFT )
		draw.DrawText( language.GetPhrase("gdr_hud_precipitation")..Precipitation, "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_Precipitation.x , pos_Precipitation.y, color, TEXT_ALIGN_LEFT )	
		draw.DrawText( language.GetPhrase("gdr_hud_incomingRadiation")..Incoming_Radiation.." W/m²", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_Incoming_Radiation.x , pos_Incoming_Radiation.y, color, TEXT_ALIGN_LEFT )
		draw.DrawText( language.GetPhrase("gdr_hud_terraintype")..Terrain_Type, "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_Terrain_Type.x , pos_Terrain_Type.y, color, TEXT_ALIGN_LEFT )			
	end
...
function hud_DrawBasicINFO()
	...
	local air_tmp   = math.Round(GetGlobalFloat("gDisasters_Revived_Temperature"),1)
	local body_tmp  = math.Round(LocalPlayer():GetNWFloat("gdr_BodyTemperature"),1)
	local body_Oxygen  = math.Round(LocalPlayer():GetNWFloat("gdr_BodyOxygen"))
	local hm        = math.Round(GetGlobalFloat("gDisasters_Revived_Humidity"))
	local windspd   = math.Round(GetGlobalFloat("gdisasters_revived_wind"),1)
	local winddir   =  math.Round(gDisasters_Revived.convert_VectorToAngle(-GetGlobalVector("gDisasters_Revived_wind_Direction")).y)
	...
	local function windspeed_Format(speed)
		...
		if speed <= 256 and GetConVar("gdisasters_revived_hud_windtype"):GetString() == "km/h" then 
			strspeed = chr1..chr2..chr3..chr4.." km/h"
		elseif speed > 256 and GetConVar("gdisasters_revived_hud_windtype"):GetString() == "km/h" then
			strspeed = table.Random({chr1..chr2..chr3..chr4.." km/h", "ERROR"})
		elseif speed <= 256 and GetConVar("gdisasters_revived_hud_windtype"):GetString() == "mph" then
			strspeed = gDisasters_Revived.convert_KMPHtoMPH(chr1..chr2..chr3..chr4).." mph"
		elseif speed > 256 and GetConVar("gdisasters_revived_hud_windtype"):GetString() == "mph" then
			strspeed = table.Random({gDisasters_Revived.convert_KMPHtoMPH(chr1..chr2..chr3..chr4).." mph", "ERROR"})
		end
	...
	local function drawText()
		...
		if GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°C" then
			draw.DrawText( language.GetPhrase("gdr_hud_air_temperature")..air_temp.."°C", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature.x , pos_air_temperature.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_body_temperature")..body_temp.."°C", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature_bh.x , pos_air_temperature_bh.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
		elseif GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°F" then
			draw.DrawText( language.GetPhrase("gdr_hud_air_temperature")..gDisasters_Revived.convert_CelciustoFahrenheit(air_temp).."°F", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature.x , pos_air_temperature.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_body_temperature")..gDisasters_Revived.convert_CelciustoFahrenheit(body_temp).."°F", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature_bh.x , pos_air_temperature_bh.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
		elseif GetConVar("gdisasters_revived_hud_temptype"):GetString() == "°K" then
			draw.DrawText( language.GetPhrase("gdr_hud_air_temperature")..gDisasters_Revived.convert_CelciustoKelvin(air_temp).."°K", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature.x , pos_air_temperature.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
			draw.DrawText( language.GetPhrase("gdr_hud_body_temperature")..gDisasters_Revived.convert_CelciustoKelvin(body_temp).."°K", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_air_temperature_bh.x , pos_air_temperature_bh.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
		end
		
		draw.DrawText( language.GetPhrase("gdr_hud_body_oxygen")..body_Oxy.."%", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_dew_point.x , pos_dew_point.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
		draw.DrawText( language.GetPhrase("gdr_hud_humidity")..humidity.."%", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_wind_chill.x , pos_wind_chill.y, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
		...
		draw.DrawText( language.GetPhrase("gdr_hud_wind_speed")..wind_speed, "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_windspeed.x , pos_windspeed.y, color, TEXT_ALIGN_LEFT )
		draw.DrawText( language.GetPhrase("gdr_hud_local_wind_speed")..local_wspeed, "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_lwindspeed.x , pos_lwindspeed.y, color2, TEXT_ALIGN_LEFT )
		draw.DrawText( language.GetPhrase("gdr_hud_wind_drection")..Wind_Direction.."°", "gDisastersFont_"..tostring(math.Round(scale * 25)), pos_winddir.x , pos_winddir.y, color, TEXT_ALIGN_LEFT )
...
function hud_DrawSeismoGraph()
	...
	local function recordTremors()
		...
		if CurTime() >= LocalPlayer().SeismoGraph_SampleIntervalNextTime then  -- here we add sample points 
			...
			local M  =  -1+( GetGlobalFloat("gdr_seismic_activity")/12)
	...
	local function drawTremors()
		...
		draw.DrawText( " Richter Scale \n     ≈  "..math.Round(GetGlobalFloat("gdr_seismic_activity"),1), "gDisastersFont_"..tostring(math.Round(scale * 15)), peak_x_min , (max_a  * (-0.5 * (ymax - ymin)))+ (0.5 * (ymax + ymin)) , Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT )
